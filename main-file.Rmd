...
m5 <- glm(impact~ neg1 +neg3 +pos1 +pos2 + `Social interactions` +`smog-index`  + neg2 + `total-words`,family=binomial(link = "logit"), data=train)
...
xyplot(log(fitted.values(m1)/(1-fitted.values(m1)))~impact, data=ldata2, type=c("l"),ylab="log odds")
xyplot(fitted.values(m1)/(1-fitted.values(m1))~impact, data=ldata2, ylab="odds")
...
library(ROCR)
p <- predict(m1, newdata=subset(test,select=c(2,3,4,5,6,7,8)), type="response")
...
testm1<-glm(all1~.,family=binomial(link="logit"),data=test)

step(glm(all1~.,family=binomial(link="logit"),data=test), scope=list(upper=m1), direction="both",test="Chisq",data=Data)

testm2 <- glm(all1~`Avg. views ret. vis.`+neg3+neg4+pos2+pos1+neg1+neg2+`total-sentences`,family=binomial(link = "logit"), data=test)
summary(testm2)
```
