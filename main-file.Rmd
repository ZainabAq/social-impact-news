...
set.seed(5)
fitControl = trainControl(method="cv", summaryFunction =  twoClassSummary, classProbs = TRUE, number = 5, returnResamp = "all")
mp6 <- train(Impact~.-URL -Title -impact -Engaged_hours -Returning_vis_10000 -Search_refs_10000 -Internal_refs_10000 -Other_refs_10000 -Direct_refs_10000 -Fb_refs_10000 -Tw_refs_10000 -Fb_interactions_10000 -total_words_10, data=pf, method="gbm", trControl=fitControl, tuneGrid = tune, verbose=FALSE,  metric = "ROC", distribution = "bernoulli")
mp6
varImp(mp6)
...
save(mp6, file = "boostingFINALFINAL.rda")
```
...
ldata_1[is.na(ldata_1)]<-0
a_train <- ldata_1[train_ind, ]
a_test <- ldata_1[-train_ind, ]
train_labels <- ldata_1[train_ind, 1]
test_labels <- ldata_1[-train_ind, 1]
train_labels[is.na(train_labels)]<-0
test_labels[is.na(test_labels)]<-0
test_pred <- knn(train = a_train, test = a_test, cl = train_labels, k=10)


#Refit model on only those classified as `1` to make up for low sensitivity
ldata$pred_add <- predict(mod_fit,nedata=ldata)
...
#split into train and test
train <- ldata_1[1:132,]
test <- ldata_1[133:232,]

m1_1 <- glm(impact~.,family=binomial(link = "logit"), data=train)
step(m1,direction="both")

m5_1<-glm(formula = impact ~ Views + Engaged_minutes + New_vis. + Avg._views_new_vis. + 
    Avg._minutes_new_vis. + Desktop_views + Internal_refs + Fb_interactions + 
    smog_index + total_words + neg1 + neg2 + neg3 + neg4 + neg5 + 
    pos1 + pos2 + pos3 + pos4, family = binomial(link = "logit"), 
    data = train)
...
xyplot(log(fitted.values(m5_1)/(1-fitted.values(m5_1)))~impact, data=ldata, type=c("l"),ylab="log odds")
xyplot(fitted.values(m5_1)/(1-fitted.values(m5_1))~impact, data=ldata, ylab="odds")
...
mod_fit <- train(impact ~ Views + Engaged_minutes + New_vis. + Avg._views_new_vis. + Avg._minutes_new_vis. + Desktop_views + Internal_refs + Fb_interactions + smog_index + total_words + neg1 + neg2 + neg3 + neg4 + neg5 + pos1 + pos2 + pos3 + pos4,  data=ldata, method="glm", family="binomial", trControl = ctrl, tuneLength = 5)

pred <- predict(mod_fit, newdata=test)

cfm1_1<-confusionMatrix(data=pred, test$impact)
cfm1_1
```

#Data Cleaning, adding variables for odds ratio
```{r}
new <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/politics-data-for-model-final2 (2).csv")
...
write.csv(new,"C:/Users/choco/Desktop/SPRING 2018/Capstone/ADDED_politics-data-for-model-final2.csv",row.names=FALSE)
...
# Final SVM
```{r}
data2 <- read_csv("FINAL_DATA_FOR_SQL.csv")
data2 = data2 %>%
  select(-Title, -URL, -ImpactBinary)

data2$impact <- as.factor(data2$impact)

set.seed(25)
sample = sample.split(data$impact, SplitRatio = .60)
data$impact <- as.factor(data$impact)
names(data)
data[,2:42] <- data.frame(lapply(data[,2:42], function(x) scale(x, center = FALSE, scale = max(x, na.rm = TRUE)/100)))
#data[, 35:42] <- lapply(data[, 35:42], as.factor)
...
obj <- tune(svm, impact~., data = train,  metric = "ROC",probability=TRUE, ranges = list(gamma = gammalist, cost = 2^(2:4)), tunecontrol = tune.control(sampling = "fix") )
...
bestmod2 = obj$best.model

class_pred = predict(bestmod, train, probability=TRUE)
prob = attributes(class_pred)
prob_all = prob[4]
pred_final = prob_all[[1]][,1]

class_pred = predict(bestmod2, train, probability=TRUE)
prob = attributes(class_pred)
prob_all = prob[4]
pred_final2 = prob_all[[1]][,1]

classification = predict(bestmod, train)
classification2 = predict(bestmod2, train)
confusionMatrix(classification, train$impact)
confusionMatrix(classification2, train$impact)

pred = ROCR::prediction(pred_final, data2$impact)
pred2 = ROCR::prediction(pred_final2, data2$impact)
...
w <- t(bestmod2$coefs) %*% bestmod2$SV                 # weight vectors
...
#testing_data$impact = as.factor(testing_data$impact)
class_pred = predict(bestmod, testing_data, probability=TRUE)

#class_pred_train = predict(bestmod, training_data)
table(predicted = class_pred, true = testing_data$impact)

class_pred<- as.numeric(class_pred)
#class_pred_train <- as.numeric(class_pred_train)
testing_data$impact <- as.numeric(testing_data$impact)
#training_data$impact <- as.numeric(training_data$impact)
rmse(class_pred, testing_data$impact)
#rmse(class_pred_train, training_data$impact)
```

# Trying a bunch of different models and looking at kapps
```{r}
# prepare training scheme
control <- trainControl(method="repeatedcv", number=10, repeats=3)
# CART
set.seed(7)
fit.cart <- train(impact~., data=training_data, method="rpart", trControl=control)
# SVM
set.seed(7)
fit.svm <- train(impact~., data=training_data, method="svmRadial", trControl=control)
# kNN
set.seed(7)
fit.knn <- train(impact~., data=training_data, method="knn", trControl=control)
# Random Forest
set.seed(7)
fit.rf <- train(impact~., data=training_data, method="rf", trControl=control)
# train SVM radial sigma
fit.svmR <- train(impact~., data=training_data, method="svmRadialSigma", trControl=control)
# collect resamples
results <- resamples(list(CART=fit.cart, SVM=fit.svm, KNN=fit.knn, RF=fit.rf, svmR=fit.svmR))
...
cart.pred=predict(fit.cart, testing_data)
table(predicted = cart.pred, true = testing_data$impact)
cart.pred<- as.numeric(cart.pred)
testing_data$impact <-as.numeric(testing_data$impact)
rmse(cart.pred, testing_data$impact)

svm.pred=predict(fit.svm, testing_data)
table(predicted = svm.pred, true = testing_data$impact)
svm.pred<- as.numeric(svm.pred)
rmse(svm.pred, testing_data$impact)

knn.pred=predict(fit.knn, testing_data)
table(predicted = knn.pred, true = testing_data$impact)
knn.pred<- as.numeric(knn.pred)
rmse(knn.pred, testing_data$impact)

rf.pred=predict(fit.rf, testing_data)
table(predicted = rf.pred, true = testing_data$impact)
rf.pred<- as.numeric(rf.pred)
rmse(rf.pred, testing_data$impact)

svmR.pred=predict(fit.svmR, testing_data)
table(predicted = svmR.pred, true = testing_data$impact)
svmR.pred<- as.numeric(svmR.pred)
rmse(svmR.pred, testing_data$impact)
```
