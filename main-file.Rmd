...
##Filtering by work time
Turk30<-Turk %>%
  filter(WorkTimeInSeconds >= 15)
...
corTurk <- tidyTurk %>%
  mutate(y1n = as.numeric(y1), y2n = as.numeric(y2), y3n = as.numeric(y3)) %>%
  select(y1n, y2n, y3n)

##Correlation matrix 
check <- cor(as.matrix(corTurk), use = "complete.obs")
check
check_rank <- rcorr(as.matrix(corTurk))
check_rank

##Checking for how many articles distinct workers worned on 
WorkerCheck <- Turk %>%
  group_by(WorkerId) %>%
  summarise(N = n())


##Rearanging Rows of URL 
Redo_Turk <- Turk[sample(1:nrow(Turk)), ]
tidy_redo <- Redo_Turk %>% 
  mutate(y = ifelse(Answer.categories == "category 1", 0, 1)) %>%
  group_by(Input.URL) %>%
  summarise(ys = paste(y, collapse = ","))%>%
...
##Creating dataframe for correlation matrix
corRedo_Turk <- tidy_redo %>%
  mutate(y1n = as.numeric(y1), y2n = as.numeric(y2), y3n = as.numeric(y3)) %>%
  select(y1n, y2n, y3n)

##Correlation matrix 
check <- cor(as.matrix(corRedo_Turk), use = "complete.obs")
check
check_rank <- rcorr(as.matrix(corRedo_Turk))
check_rank

##Creating random values for Ys 
randomTurk <- data.frame(URL = Turk$Input.URL, yR = sample(0:1, length(Turk$Input.URL), replace = TRUE))
randomTurk <- randomTurk %>% 
  group_by(URL) %>%
  summarise(ys = paste(yR, collapse = ","))%>%
  separate(ys, into = c("y1", "y2", "y3"))


##Creating dataframe for correlation matrix
corRandomTurk <- randomTurk %>%
  mutate(y1n = as.numeric(y1), y2n = as.numeric(y2), y3n = as.numeric(y3)) %>%
  select(y1n, y2n, y3n)

##Correlation matrix 
check2 <- cor(as.matrix(corRandomTurk), use = "complete.obs")
check2
check_rank2 <- rcorr(as.matrix(corRandomTurk))
check_rank2

###Merging Turk 
Turk3 <- tidyTurk %>%
  mutate(all1 = ifelse(y1 == 1 & y2 == 1 & y3 == 1, 1, 0))%>%
  rename(URL = Input.URL)
politics_2500_text <- read_csv("text-data-nlp-politics.csv")
politics_2500_text = politics_2500_text[1:2500,]
politics_3 <- politics_[1:2500,]
politics_2500 <- left_join(politics_2500_text, politics_3, by = "URL")
politics_2500 <- left_join(politics_2500, Turk3, by = "URL")
added_sent_2500 <- read_csv("politics_added_sentiment.csv")
added_sent_2500 = added_sent_2500 %>%
  select(URL, neg1, neg2, neg3, neg4, neg5, pos1, pos2, pos3, pos4, pos5)
added_sent_2500 = added_sent_2500[1:2500,]
politics_2500 <- left_join(politics_2500, added_sent_2500, by = "URL")
write.table(politics_2500, file="politics-2500-merged.csv", sep =",", row.names=F)
politics_2500 =politics_2500 %>%
  select(-ID, -URL, -text, -`Publish date`, -Authors, -Section, -Tags, -`Sort (Views)` , -Title, -y1, -y2, -y3)
politics_2500[is.na(politics_2500)] <- 0

##Cleaning data 
spaceless <- function(x) {colnames(x) <- gsub(" ", "_", colnames(x));x}
hless <- function(x) {colnames(x) <- gsub("-", "_", colnames(x));x}
politics_2500 <- spaceless(politics_2500)
politics_2500 <- hless(politics_2500)
names(politics_2500)
#politics_300 <- politics_300 %>%
  #mutate(metric = Social_interactions/Views)

##Classification Tree of the whole dataset
tree_p2 <- tree(as.factor(all1) ~., politics_2500)
summary(tree_p2)
plot(tree_p2)
text(tree_p2, pretty=0)

##Training and Testing dataset

set.seed(5)
politics_train = politics_2500 %>%
  sample_frac(0.5)

politics_test = politics_2500 %>%
  setdiff(politics_train)


##Classication tree
set.seed(5)
mp21 <- tree(as.factor(all1) ~ . , data=politics_train)
summary(mp21)
plot(mp21)
text(mp21, pretty = 0)
mp21_estimates = predict(mp21, politics_test, type = "class")
table(mp21_estimates, politics_test$all1)
(22+19)/64 ##64.1% correct predictions 

##Pruned tree with CV 
set.seed(5)
mp23 <- cv.tree(mp21, FUN = prune.misclass)
plot(mp23$size, mp23$dev, type = "b")
mp24 <- prune.misclass(mp21, best = 5)
plot(mp24)
text(mp24, pretty = 0)
mp24_estimates = predict(mp24, politics_test, type = "class")
table(mp24_estimates, politics_test$all1)
(22+21)/64 ##67.2% correct predictions

##Random Forest model with Conditional inference (uses statistical test rather than purity)
set.seed(5)
mp25 <- randomForest(as.factor(all1) ~. , data=politics_train, controls=cforest_unbiased(ntree=2000, mtry=5))
varImpPlot(mp25)
mp25_estimates = predict(mp25, newdata = politics_test, type = "response") 
table(mp25_estimates, politics_test$all1)
(61+16)/115 ##67 correct predictions



##Boosting with Classification 
mp26<- gbm(all1 ~., data = politics_2500, distribution = "bernoulli", n.trees = 2000, cv.folds = 5, verbose = F)
best.iter = gbm.perf(mp26, method = "cv")
