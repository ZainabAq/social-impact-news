...
confusionMatrix(data=pred, test$impact)
```

```{r}
#random data logistic regression
test <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/politics-2500-merged (2).csv") %>%
  select(-ID,-URL,-Title,-text,-y1,-y2,-y3,-Authors,-Section,-Tags,-`Li refs`,-`Pi refs`,-`Li interactions`,-`Pi interactions`,-`Publish date`)

test[is.na(test)]<-0

#split into train and test
training <- test[1:1500,]
testing <- test[1501:2500,]

testm1<-glm(all1~.,family=binomial(link="logit"),data=training)

step(glm(all1~.,family=binomial(link="logit"),data=test), scope=list(upper=testm1), direction="both",test="Chisq",data=Data)
...
library(MKmisc)
HLgof.test(fit = fitted(testm2), obs = testing$all1)

library(ResourceSelection)
hoslem.test(testing$all1, fitted(testm2), g=10)
