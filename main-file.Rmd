...
```{r}
ldata <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/log data (politics-2500-merged).csv")

ldata <- ldata %>%
  select(-ID,-text,-Title, -`Publish date`, -Authors,  -Section, -Tags, -y1, -y2, -y3) %>%
  filter(I)

missmap(ldata, main = "Missing values vs observed")

ldata <- ldata %>%
  select(-`Li refs`, -`Pi refs`, -`Li interactions`, -`Pi interactions`)
...
train <- ldata[1:1250,]
test <- ldata[1251:2500,]

m1 <- glm(all1~.-URL,family=binomial("logit"), data=train)

#m1 <- logistf(all1 ~.,family=binomial(link='logit'),data=train)
