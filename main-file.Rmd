...
MK <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/MK_politics_150_v2.csv")
JA <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/JA_politics_150_v2.csv")
ZA <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/ZA_politics_150_v2.csv")
EF <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/EF_politics_150_v2.csv")

rated_200 <- rbind(MK, JA)
rated_200 <- rbind(rated_200,ZA)
rated_200 <- rbind(rated_200,EF)

past <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/past_politics_300.csv")

rated_200 <- rbind(rated_200,past)

write.csv(rated_200, file = "C:/Users/choco/Desktop/SPRING 2018/Capstone/politics_rated_for_s3.csv")
```
...
```{r}
data_p <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/text-data-nlp-politics.csv")
...
test<-read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/Batch_3145287_batch_results.csv")

target <- test %>%
  filter(WorkerId=="A3QC57KUVJP5EW"|"A6NH7ITRPQ4O1")
```
...
ldata<-read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/politics-2500-merged (2).csv")

ldata <- ldata %>%
  select(-ID, -text, -y1, -y2, -y3, -Title, -Authors, -`Publish date`, -Section, -Tags)

ldata[is.na(ldata)] <-0

log <- glm(all1~., data=ldata, family="binomial")



wss <- (nrow(ld2)-1)*sum(apply(na.omit(ld2),2,var))
for (i in 1:6) wss[i] <- sum(kmeans(ld2, 
                                     centers=i)$withinss)
...
# Read the Data
data_EF <- read_csv("Raw data_10000 articles_Jul-Dec 2017.csv") %>% #import original data
  select(-URL, -Title, -`Publish date`, -Authors, -Section, -Tags)

# Random sampling
samplesize = 0.60 * nrow(data_EF)
set.seed(1)
index = sample( seq_len ( nrow ( data_EF ) ), size = samplesize )

# Create training and test set
datatrain = data_EF[ index, ]
datatest = data_EF[ -index, ]

max = apply(data_EF , 2 , max)
min = apply(data_EF, 2 , min)
scaled = as.data.frame(scale(data_EF, center = min, scale = max - min))
...
# fit neural network
set.seed(2)
NN = neuralnet(`Sort (Views)`~Visitors+Views, trainNN, hidden = 3 , linear.output = T,data=data_EF )
...
predict_testNN = (predict_testNN$net.result * (max(data_EF$rating) - min(data_EF$rating))) + min(data_EF$rating)
...
text1 <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/politics-2500-merged (2).csv")
text1 <- text1 %>%
  select(URL, text)
text1$text<- str_replace_all(text1$text, "[[:punct:]]", "") #omit special characters

#text1 <- str_replace_all(text1$text, "<U+06EA>", "'")

write.csv(text1, file = "C:/Users/choco/Desktop/SPRING 2018/Capstone/politics_new_amazon.csv")

```

```{r}
#900 data logistic regression
data <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/politics-data-for-model.csv")

ldata <- data

missmap(ldata, main = "Missing values vs observed")

ldata <- ldata %>%
  select(-Li_refs,-Pi_refs,Li_interactions,-Pi_interactions)
...
#m2 <- glm(impact~`Direct refs`+`Sort (Views)` + neg1 +neg3 +pos1 +pos2 + `Social interactions` +`smog-index` +`Avg. views ret. vis.` + neg2 + `total-words`,family=binomial(link = "logit"), data=train)

#m3 <- glm(impact~`Sort (Views)` + neg1 +neg3 +pos1 +pos2 + `Social interactions` +`smog-index` +`Avg. views ret. vis.` + neg2 + `total-words`,family=binomial(link = "logit"), data=train)

m5 <- glm(impact~ neg1 +neg3 +pos1 +pos2 + Social_interactions +smog_index  + neg2 + total_words,family=binomial(link = "logit"), data=train)
...
```{r}
#random data logistic regression
randomdata <- read_csv("C:/Users/choco/Desktop/SPRING 2018/Capstone/politics-2500-merged (2).csv") %>%
  select(-ID,-URL,-Title,-text,-y1,-y2,-y3,-Authors,-Section,-Tags,-`Li refs`,-`Pi refs`,-`Li interactions`,-`Pi interactions`,-`Publish date`)

randomdata[is.na(randomdata)]<-0

#split into train and randomdata
training <- randomdata[1:1500,]
testing <- randomdata[1501:2500,]

testm1<-glm(all1~.,family=binomial(link="logit"),data=training)

step(glm(all1~.,family=binomial(link="logit"),data=randomdata), scope=list(upper=testm1), direction="both",randomdata="Chisq",data=Data)

testm2 <- glm(all1~`Avg. views ret. vis.`+neg3+neg4+pos2+pos1+neg1+neg2+`total-sentences`,family=binomial(link = "logit"), data=testing)
summary(testm2)

pR2(testm2)

library(MKmisc)
HLgof.randomdata(fit = fitted(testm2), obs = testing$all1)

library(ResourceSelection)
hoslem.randomdata(testing$all1, fitted(testm2), g=10)

library(survey)
regTermTest(testm2, "neg3")

varImp(testm2)

pred = predict(testm2, newdata=testing)
accuracy <- table(pred, testing[,"all1"])
sum(diag(accuracy))/sum(accuracy)

library(ROCR)
# Compute AUC for predicting Class with the model
prob <- predict(testm2, newdata=testing, type="response")
pred <- prediction(prob, testing$all1)
perf <- performance(pred, measure = "tpr", x.measure = "fpr")
plot(perf)
auc <- performance(pred, measure = "auc")
auc <- auc@y.values[[1]]
...
testing$all1<-as.factor(testing$all1)
randomdata$all1 <- as.factor(randomdata$all1)

ctrl <- trainControl(method = "repeatedcv", number = 10, savePredictions = TRUE)

mod_fit <- train(all1~`Avg. views ret. vis.`+neg3+neg4+pos2+pos1+neg1+neg2+`total-sentences`,  data=randomdata, method="glm", family="binomial",
                 trControl = ctrl, tuneLength = 5)


pred = predict(mod_fit, newdata=randomdata)
cfm2<-confusionMatrix(data=pred, randomdata$all1)
