...
abstract: |
  Our team used articles from a major Online news publication and tried to create a tool that could automatically classify socially impactful articles from those that are not. Socially impactful articles are those that can change the way readers think, leave an impression on them, or reaffirm their ideas. It could also make readers change their habits or actions based on what they have read. Some variables we analyzed include social media (e.g. Facebook referrals), article readability and sentiment. We trained three models (tree, SVM and logistic regression) to have several approaches in identifying socially impactful articles. We created a tool through Shiny, a web application framework, to create a user-friendly interface that displays the results of these models plus an average of the three. Our tool provides several scores that serve as simple guidelines for readers curious to know the probability of an article being socially impactful or not without reading its contents. It also provides information on the overall strengths and emphasis on each model and what the distribution of variables are. Through this information, users are able to decide for themselves the best model score to use for a particular article. 
